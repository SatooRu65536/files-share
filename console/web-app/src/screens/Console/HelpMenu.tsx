...
const HelpMenu = () => {
  ...
  const helpTabName = useSelector((state: AppState) => state.system.helpTabName);
  ...
  return (
    <Fragment>
          ...
          <Tabs
            ...
            optionsInitialComponent={
              <Box sx={{ margin: '10px 10px 10px 15px' }}>
                <HelpIconFilled style={{ color: '#3874A6', width: 16, height: 16 }} />
              </Box>
            }
            optionsEndComponent={
      ...
      <Button id={systemHelpName ?? 'help_button'} icon={<HelpIcon />} onClick={toggleHelpMenu}></Button>
    </Fragment>
  );
};
