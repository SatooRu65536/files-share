...
import { cleanList, deleteFromList } from '../../ObjectBrowser/objectBrowserSlice';
    ...
    <Fragment>
      {managerOpen && (
        <Box
          ...
          className={`${anonymousMode ? 'downloadContainerAnonymous' : ''} ${managerOpen ? 'open' : ''}`}
          useBackground
          withBorders
        >
          <Box
          ...
          >
            <Tooltip tooltip={'Clean Completed Objects'} placement="bottom">
              <IconButton aria-label={'Clear Completed List'} onClick={() => dispatch(cleanList())}>
                <RemoveAllIcon />
              </IconButton>
            </Tooltip>
          </Box>
          ...
          >
            <VirtualizedList rowRenderFunction={renderObject} totalItems={objects.length} defaultHeight={110} />
          </Box>
        </Box>
      )}
    </Fragment>
