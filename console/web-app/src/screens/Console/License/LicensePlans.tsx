...
const LicensePlans = () => {
  const currentPlan = 'community';

  const getButton = (link: string, btnText: string, variant: any) => {
    const linkToNav = currentPlan !== 'community' ? 'https://subnet.min.io' : link;
    return (
      <Button
            ...
            <Box className={`first ${index === 1 ? 'openSource' : index === 2 ? 'enterprise' : ''}`} key={index}>
              {element !== null && (
                <Box>
                  ...
                  <Box className={`planIcon ${element.planType === 'commercial' ? 'commercial' : ''}`}>
                    {element?.planIcon}
                  </Box>
                  <Box className={'planDescription'}>{element?.planDescription}</Box>
                </Box>
              )}
            </Box>
          );
