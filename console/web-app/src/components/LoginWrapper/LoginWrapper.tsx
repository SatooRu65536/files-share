...
import { ApplicationLogo, breakPoints, Grid } from 'mds';
...
const CustomLogin = styled.div(({ theme }) => {
  return {
    ...
    '& .decorationPanel': {
      ...
      '& .videoContainer': {
        ...
        '&:before': {
          ...
          display: 'block',
          content: "' '",
          background: 'linear-gradient(to bottom, rgba(0,1,16,1) 0%,rgba(0,0,0,0.02) 100%)',
          top: 0,
        },
        '&:after': {
          position: 'absolute',
          ...
          display: 'block',
          content: "' '",
          background: 'linear-gradient(to right, rgba(0,1,16,1) 0%,rgba(0,0,0,0.02) 100%)',
          top: 0,
        },
        '& .videoBG': {
          width: '100%',
      ...
      '& .bgExtend': {
        backgroundImage: 'linear-gradient(45deg,rgba(172,223,234,0) 0,#7fc0e4 100%)',
        position: 'absolute',
        width: 500,
        left: 0,
      },
    ...
    '& .formPanel': {
      ...
      [`@media (min-width: ${get(breakPoints, 'xs', 0)}px) and (max-width: ${get(breakPoints, 'md', 0)}px)`]: {
        maxWidth: '100%',
      },
      '& .logoContainer': {
        display: 'flex',
      ...
      '& .formContainer': {
        ...
        '& .footer': {
          display: 'flex',
          width: '328px',
          borderTop: `${get(theme, 'login.footerDivider', '#f2f2f2')} 1px solid`,
          padding: '35px 0',
          textAlign: 'center',
          alignItems: 'flex-end',
          justifyContent: 'center',
        ...
        <Grid item xs={'hidden'} sm={'hidden'} md className={'decorationPanel'}>
          ...
          <Grid item className={'videoContainer'}>
            {GPUAvailable && backgroundAnimation ? (
              <video autoPlay playsInline muted loop disablePictureInPicture poster={poster} className={'videoBG'}>
